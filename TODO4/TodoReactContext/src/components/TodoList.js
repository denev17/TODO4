...
const TodoList = () => {
  ...
  return ( 
    <div className="flex items-center justify-center h-screen" style={{ backgroundImage: 'url(/bg3.jpg)', backgroundSize: 'cover' }}>
      <div className="w-full max-w-lg px-4 py-16 sm:px-0">
        <Tab.Group onChange={(index) => setSelectedTab(index === 0 ? 'Todo List' : 'Completed')}>
          <Tab.List className="flex space-x-4 rounded-xl bg-red-900/20 p-2 w-full">
            ...
            {Object.entries(categories).map(([tabName, posts], idx) => (
              ...
              >
                ...
                {tabName === 'Completed' && posts.length === 0 ? (
                  <p className="text-pink-700">There's no task completed yet.</p>
                ) : (
                  <ul className="overflow-y-auto max-h-96">
                    {posts.map((post) => (
                      <li
                        key={post.id}
                        className={classNames(
                          'relative rounded-md p-4 hover:bg-pink-300',
